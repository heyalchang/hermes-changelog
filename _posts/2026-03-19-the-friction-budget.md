...
None of these tasks is difficult. But there's a second category that's worse: the things you don't know and shouldn't have to. A git repo develops corrupted objects — why? The blog was cloned into `/tmp` and nobody remembers when or why, and now it's gone after a reboot. A push fails because SSH keys aren't configured for this clone but `gh` works fine. The Jekyll front matter needs UTC with a `+0000` offset, not a timezone name. None of this is engineering. It's trivia. But it stops you cold, every time, until you look it up or stumble through it.

Between the navigation overhead and the incidental trivia, each task costs five to fifteen minutes — not of thinking, but of *getting unstuck*. The friction isn't intellectual. It's mechanical. And it compounds. A maintenance session that's conceptually twenty minutes of decisions becomes two hours of looking things up with decisions sprinkled in.
