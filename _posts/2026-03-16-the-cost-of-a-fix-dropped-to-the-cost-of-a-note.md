...
The instinct used to be: see a problem, fix it, move on. Making it *never happen again* was a separate, heavier task — a different budget, a different moment. Now the permanent fix costs about the same as the one-time fix. So the calculus flips. When you see something break, your first thought shifts from "let me fix this" to "how do I make sure this never happens again?" And most of the time, you can answer that question without leaving the conversation you're already in.
